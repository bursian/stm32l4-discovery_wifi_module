...
int ism43362_init() {
	...
	for (clock_t t0 = clock(); !WIFI_IS_CMDDATA_READY(); usleep(5000)) if ((clock() - t0) > 10000) return -10001;	// No CMD/DATA_READY sign after reset
	...
	for (int i = 0; WIFI_IS_CMDDATA_READY(); i++) {
		if (spi_transfer(spi_dev, (uint8_t*)pad, (uint8_t*)in, 1) < 0) {WIFI_CHIP_DESELECT(); return -10002;}	// SPI transfer error while reading initial prompt
		if (i > 5) {WIFI_CHIP_DESELECT(); return -10003;}							// Too many characters while reading initial prompt
	}
	WIFI_CHIP_DESELECT();

	if ((in[0] != '>') || (in[1] != ' ')) return -10004;								// Initial prompt not valid
		
	for (clock_t t0 = clock(); !WIFI_IS_CMDDATA_READY(); usleep(1000)) if ((clock() - t0) > 100) return -10005;	// No CMD/DATA_READY sign after initial prompt
...
int ism43362_exchange(char *txb, int txl, char *rxb, int rxl) {
	...
	if (txb && txl) {
		...
		for (clock_t t0 = clock(); !WIFI_IS_CMDDATA_READY(); usleep(1000)) if ((clock() - t0) > 100) return -10006;		// No CMD/DATA_READY sign for more than 100 ms
		
		// Send command
		WIFI_CHIP_SELECT();
		if (spi_transfer(spi_dev, (uint8_t*)txb, (uint8_t*)rxb, txl/2) < 0) {WIFI_CHIP_DESELECT(); return -10007;}		// SPI transfer error while sending command
		if (txl & 1) if (spi_transfer(spi_dev, (uint8_t*)tail, (uint8_t*)in, 1) < 0) {WIFI_CHIP_DESELECT(); return -10007;}	// SPI transfer error while sending command
		WIFI_CHIP_DESELECT();
		...
		for (clock_t t0 = clock(); WIFI_IS_CMDDATA_READY();) if ((clock() - t0) > 10) return -10008;				// Error waiting CMD/DATA READY sign disappears
	}
	...
	for (clock_t t0 = clock(); WIFI_IS_CMDDATA_READY();) {
		if (rxc > rxl-2) {WIFI_CHIP_DESELECT(); return -rxc;}									// rxb is too small for full answer
		if ((clock() - t0) > 1000) {WIFI_CHIP_DESELECT(); return -10010;}							// No end of answer for more than 1 s	
		if (spi_transfer(spi_dev, (uint8_t*)pads, (uint8_t*)in, 1) < 0) {WIFI_CHIP_DESELECT(); return -10011;}			// SPI transfer error while reading answer
		rxb[rxc++] = in[0];
		rxb[rxc++] = in[1];
	}
	WIFI_CHIP_DESELECT();
